...
class Thread::Pool
    ...
    def report_gui_timeout()
		...
		def execute (thread)
          ...
          #report_gui_timeout()
          #excep = BotThreadTimeout.new( reason.message )
          #excep.set_backtrace( reason.backtrace )
          #excep.report
		...
		def timeout!
       report_gui_timeout()
			 begin terminate! Timeout ; rescue => err ; end
		end
...
private
	...
	def spawn_timeout_thread
        ...
        @mutex.synchronize {
            ...
            task.thread.exit if task.thread

            @spawned -= 1
            @workers.delete task.thread if task.thread
